...
\begin{center}
  ...
  \begin{tabular}{ll}
    ...
    \textbf{Date:} & \FILL{submission date}\\
  \end{tabular}
\end{center}
\vspace{6pt}
\hrule
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Machine} & \textbf{Role} & \textbf{OS} & \textbf{Address (fill in)}\\
\midrule
PC~1 & Video server (victim) & \FILL{Windows/Linux ver.} & IP \FILL{server IP}, MAC \FILL{server MAC}\\
PC~2 & Client / player (victim) & \FILL{Windows/Linux ver.} & IP \FILL{client IP}, MAC \FILL{client MAC}\\
PC~3 & Attacker & Linux (\FILL{distro/ver.}) & IP \FILL{attacker IP}, MAC \FILL{attacker MAC}\\
\midrule
\multicolumn{4}{@{}l}{Link layer: Wi-Fi access point / router \FILL{make/model}, subnet \FILL{e.g.\ 192.168.x.0/24}}\\
\bottomrule
\end{tabular}
...
topology and Figure~\ref{fig:timeline}/Section~\ref{sec:attack-steps} the
timeline.
...
\begin{figure}[h]
\ssplaceholder{Attacker terminal: \code{run_attack.sh} output showing ``locked
onto connection \dots'' and the ``RSTs sent='' hammer counter.}{images/fig_attacker_rst.png}
\caption{The injector locking onto the victim connection and hammering forged
RSTs. Record the \code{RSTs sent=} value at teardown for Table~\ref{tab:metrics}.}
\label{fig:attacker}
\end{figure}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  \item \textbf{Clean MITM confirmed.} After poisoning, the victims' ARP tables
        showed the peer IP mapped to the \emph{attacker's} MAC, and the stream
        kept playing normally while transiting the attacker
        (Figure~\ref{fig:mitm}).
  \item \textbf{Teardown observed.} The client printed the RST banner (``reset
        with NO preceding FIN'') and then \textbf{PLAYBACK STALLED} once the
        buffer drained (Figure~\ref{fig:stall}); the \code{ESTABLISHED} entry
        disappeared from the client's connection table
        (Figure~\ref{fig:ss}).
  \item \textbf{On the wire.} The capture showed a run of DATA segments followed
        by RST(s) with \textbf{no FIN} (Figure~\ref{fig:wireshark}).
\end{itemize}

\begin{figure}[h]
\ssplaceholder{Client status lines advancing normally (buffer/playback growing)
\emph{while} the attacker is MITM — proving a clean MITM, not a DoS. Optionally
also show \code{ip neigh}/\code{arp -a} on a victim with the peer IP $\to$
attacker MAC.}{images/fig_mitm_healthy.png}
\caption{Healthy stream under an active MITM (before the RST), plus a poisoned ARP
entry on a victim.}
\label{fig:mitm}
\end{figure}

\begin{figure}[h]
\ssplaceholder{Client terminal: the ``TCP CONNECTION RESET (RST) received''
banner followed by ``PLAYBACK STALLED''.}{images/fig_client_stall.png}
\caption{The victim experiencing the outage: abrupt reset (no FIN), buffer
drains, playback stalls.}
\label{fig:stall}
\end{figure}

\begin{figure}[h]
\ssplaceholder{\code{ss -tan | grep 9000} (Linux) or \code{netstat -ano | findstr
:9000} (Windows) on the client, BEFORE (ESTABLISHED) and AFTER (gone) the
attack.}{images/fig_ss_gone.png}
\caption{The connection table on the client before vs.\ after: the
\code{ESTABLISHED} entry vanishes — hard proof the socket was destroyed.}
\label{fig:ss}
\end{figure}

\begin{figure}[h]
\ssplaceholder{Wireshark filtered on \code{tcp.port == 9000}: a run of PSH,ACK
data segments, then RST segment(s), with NO FIN. Highlight one forged RST whose
source IP is the server but which actually came from the attacker.}{images/fig_wireshark_rst.png}
\caption{Packet capture: DATA segments then a forged RST with no preceding
FIN/FIN-ACK.}
\label{fig:wireshark}
\end{figure}
...
\toprule
\textbf{Metric} & \textbf{How measured} & \textbf{Result (fill in)}\\
\midrule
Attack success rate (\%) & successful resets over $N$ trials & \FILL{e.g.\ 10/10 = 100\%}\\
Time-to-disruption & attack trigger $\to$ player stall & \FILL{e.g.\ $\sim$X s}\\
RST attempts per success & injector \code{RSTs sent=} at teardown & \FILL{e.g.\ $\sim$X}\\
Behavior vs.\ client retry & \code{RECONNECT=1}: recovers on its own? & \FILL{needs sustained injection}\\
Success with vs.\ without defense & rerun with static ARP pinned & \FILL{without: succeeds; with: fails}\\
\bottomrule
\end{tabular}
\caption{Measured results (design proposal Table~2). Repeat each run a few times
...
\begin{enumerate}[leftmargin=1.4em,itemsep=4pt]
  \item \textbf{Docker Phase~1 was not used; we demonstrated directly on three
        physical machines.} The proposal planned a single-host Docker simulation
        (Phase~1) before the physical demo (Phase~2). In practice we could not
        get the Docker-based setup working for our demonstration
        \FILL{state the concrete reason, e.g.\ Docker Desktop/WSL2 networking
        issues on the demo laptop}, so we conducted the \emph{entire} attack and
        defense on three real machines. \emph{Impact:} none on the result — the
        physical demo is the stronger evidence anyway (it proves the attack is
        not an artifact of the Docker virtual network stack), which was the whole
        purpose of Phase~2.
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{ARP monitoring (detection).} \code{arp_watch.py} (pure standard
        library, runs on any victim) polls the neighbour table and alerts on a
        watched IP's MAC changing, or on one MAC claiming several IPs (the MITM
        signature); it can optionally re-pin automatically. This is the host-side
        stand-in for DAI.
